...
export interface TeacherManualItem {
  id: number | string;
  _id?: number | string;
  title: string;
  code?: string;
...
export default function TeacherManualPage() {
  ...
  // Data States
  const [manuals, setManuals] = useState<TeacherManualItem[]>([]);
  ...
  const [editingManual, setEditingManual] = useState<TeacherManualItem | null>(null);

  const [isPreviewOpen, setIsPreviewOpen] = useState<boolean>(false);
  const [previewingItem, setPreviewingItem] = useState<TeacherManualItem | null>(null);

  const [isDeleteOpen, setIsDeleteOpen] = useState<boolean>(false);
  const [itemToDelete, setItemToDelete] = useState<TeacherManualItem | null>(null);
  ...
  const getItemId = (item: TeacherManualItem): string | number => {
  ...
  // 2. Fetch Teacher Manuals
  const fetchManuals = async (page = 1, showRefreshToast = false) => {
    if (!canAccess) return;
    ...
    try {
      ...
      let response;
      try {
        response = await api.get("/teacher-manuals", { params });
      } catch (err) {
        try {
          response = await api.get("/v1/teacher-manuals", { params });
        } catch (err2) {
          response = await api.get("/teacher_manuals", { params });
        }
      }

      const resData = response.data;
      let list: TeacherManualItem[] = [];
      let paginationMeta = {
        total: 0,
        page: 1,
        limit: itemsPerPage,
      ...
      if (showRefreshToast) {
        toast.success("Teacher Manuals refreshed");
      }
    } catch (error: any) {
      console.error("Error fetching teacher manuals:", error);
      toast.error(error?.response?.data?.message || "Failed to load teacher manuals");
    } finally {
      setIsLoading(false);
      ...
      <div className="p-4 max-w-4xl mx-auto my-6">
        <div className="bg-white rounded-2xl border border-rose-200 p-4 md:p-5 text-center shadow-lg space-y-3.5">
          ...
          <p className="text-sm text-[#505f76] max-w-md mx-auto">
            Teacher Manuals contain answer keys and pedagogical guides restricted exclusively to Teachers and Admins.
          </p>
        </div>
      </div>
    );
  ...
  // Open Edit Modal
  const handleOpenEditModal = (item: TeacherManualItem) => {
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    if (!formData.title.trim()) {
      toast.error("Please enter a manual title");
      return;
    }

    if (!editingManual && !formData.bookId) {
      toast.error("Please select a target book");
      return;
    }

    if (!editingManual && !selectedFile) {
    ...
    try {
      ...
      if (editingManual) {
        const id = getItemId(editingManual);
        try {
          await api.put(`/teacher-manuals/${id}`, submitData, {
            headers: { "Content-Type": "multipart/form-data" },
          });
        } catch (err) {
          await api.put(`/v1/teacher-manuals/${id}`, submitData, {
            headers: { "Content-Type": "multipart/form-data" },
          });
        }
        toast.success("Teacher Manual updated successfully");
      } else {
        try {
          await api.post("/teacher-manuals", submitData, {
            headers: { "Content-Type": "multipart/form-data" },
          });
        } catch (err) {
          await api.post("/v1/teacher-manuals", submitData, {
            headers: { "Content-Type": "multipart/form-data" },
          });
        }
        toast.success("Teacher Manual uploaded successfully");
      }

      setIsUploadModalOpen(false);
      fetchManuals(currentPage);
    } catch (error: any) {
      console.error("Error saving teacher manual:", error);
      toast.error(error.response?.data?.message || "Failed to save teacher manual");
    } finally {
  ...
  const handleDeleteItem = async () => {
    ...
    try {
      ...
      try {
        await api.delete(`/teacher-manuals/${id}`);
      } catch (err) {
        await api.delete(`/v1/teacher-manuals/${id}`);
      }
      toast.success("Teacher Manual deleted successfully");
      setIsDeleteOpen(false);
      setItemToDelete(null);
      fetchManuals(currentPage);
    } catch (error: any) {
      console.error("Delete failed:", error);
      toast.error(error.response?.data?.message || "Failed to delete teacher manual");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="space-y-4 md:space-y-5 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 md:p-5 rounded-2xl border border-[#c3c6d7]/40 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-[#131b2e] tracking-tight">Teacher Manuals</h1>
          </div>
          <p className="text-sm text-[#505f76] mt-1 ml-10">
            Access and manage answer keys, teaching guides, and pedagogical solutions for assigned books.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            onClick={() => fetchManuals(currentPage, true)}
      ...
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-[#c3c6d7]/40 shadow-sm space-y-3.5">
        <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center justify-between">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input
              type="text"
              placeholder="Search teacher manuals or book title..."
              value={searchQuery}
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-[#c3c6d7]/40">
          ...
          <p className="text-sm font-semibold text-[#505f76]">Loading teacher manuals...</p>
        </div>
      ) : manuals.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 bg-white rounded-2xl border border-[#c3c6d7]/40 text-center px-4">
          ...
          <h3 className="text-lg font-bold text-[#131b2e]">No Teacher Manuals Found</h3>
          <p className="text-sm text-[#505f76] max-w-md mt-1 mb-6">
            {hasActiveFilters
              ? "No manuals match your active filters. Try resetting filters."
              : "No teacher manuals have been uploaded yet."}
          </p>
                ...
                <div className="px-5 py-3.5 bg-[#faf8ff] border-t border-[#c3c6d7]/30 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => downloadPdfFile(item.downloadUrl, item.fileUrl, `${item.title || "teacher-manual"}.pdf`)}
                  ...
                  return (
                    <tr key={getItemId(item)} className="hover:bg-[#faf8ff]/60 transition-colors">
                      <td className="py-3.5 px-4 font-semibold">
                        <div className="flex items-center gap-2">
                          <BookOpenCheck className="h-4 w-4 text-purple-600 shrink-0" />
                          <div>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => downloadPdfFile(item.downloadUrl, item.fileUrl, `${item.title || "manual"}.pdf`)}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-[#c3c6d7]/40 shadow-sm text-xs">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => fetchManuals(currentPage - 1)}
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-4 md:p-5 space-y-4 border border-[#c3c6d7]/50 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-[#c3c6d7]/30 pb-3">
              <h2 className="text-lg font-bold text-[#131b2e]">
                {editingManual ? "Edit Teacher Manual" : "Upload Teacher Manual PDF"}
              </h2>
            ...
            <form onSubmit={handleSubmitForm} className="space-y-4 text-xs">
              <div>
                ...
                <select
                  ...
                  disabled={Boolean(editingManual)}
              ...
              <div>
                <label className="font-semibold text-[#131b2e] block mb-1">Manual Title *</label>
                <input
                  type="text"
                  placeholder="e.g. Class 1 English Teacher Manual"
              ...
              <div>
                <label className="font-semibold text-[#131b2e] block mb-1">Manual Code (Optional)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-semibold text-[#131b2e] block mb-1">
                  Upload PDF File {editingManual ? "(Optional replacement)" : "*"}
                </label>
                <div className="border-2 border-dashed border-[#c3c6d7]/80 hover:border-[#004ac6] rounded-2xl p-4 text-center bg-[#faf8ff] transition-colors relative cursor-pointer">
                  ...
                  />
                  <UploadCloud className="h-8 w-8 text-purple-600 mx-auto mb-1" />
                  {selectedFile ? (
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Pedagogical guidelines and lesson answers..."
                ...
                <Button
                  ...
                  onClick={() =>
                    downloadPdfFile(
                      ...
                      `${previewingItem.title || "teacher-manual"}.pdf`
                    )
                  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-md w-full p-4 md:p-5 space-y-3.5 border border-[#c3c6d7]/50 shadow-xl">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <h3 className="text-lg font-bold text-[#131b2e]">Delete Teacher Manual</h3>
            </div>

            <p className="text-xs text-[#505f76] leading-relaxed">
              Are you sure you want to delete <strong>&quot;{itemToDelete.title}&quot;</strong>? This will permanently delete the manual record and PDF file.
            </p>
            ...
            <div className="flex items-center justify-end gap-3 pt-3">
              ...
              <Button onClick={handleDeleteItem} disabled={isDeleting} className="bg-rose-600 hover:bg-rose-700 text-white">
                {isDeleting ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : "Yes, Delete Manual"}
              </Button>
            </div>
          </div>
        </div>
