...
export default function ChaptersPage() {
  ...
  const fetchFilteredBooks = async () => {
    try {
      ...
      // Auto-set or preserve selectedBookId
      if (normalized.length > 0) {
        const exists = normalized.some((b) => String(b.id) === String(selectedBookId));
        if (!exists) {
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#faf8ff] border-b border-[#c3c6d7]/30 text-[11px] font-extrabold uppercase tracking-wider text-[#505f76]">
                  ...
                  <th className="py-4 px-6">Chapter Title & Description</th>
